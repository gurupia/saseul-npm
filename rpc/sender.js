...
module.exports = class Sender {
    ...
    SendTransactionWithResult(signedTransaction, waitMicroSeconds = 60000000, blockSearchPadding = 10) {
        try {
            return this.SendTransaction(signedTransaction.data, signedTransaction.public_key, signedTransaction.signature).then(async res => {
                if(res.code !== CODE.OK || res.status !== 'success' || res.msg !== 'Transaction is added') throw res;
    ...
    Checker = {
        /**
         * Get byte fee of the contract
         * @param contractType : string
         * @returns {Promise<string>}
         * @constructor
         */
        ByteFee: async (contractType) => {
            try {
                let feeAmount = null;
                const ctype = 'contract';
                const cid = Hasher.IdHash([contractType, Config.SystemNonce()]);
                ...
                if(getCode.data[contractIndex] === undefined || getCode.data[contractIndex] === null) throw `Contract "${contractType}" doesn\'t exist. `;

                const contract = JSON.parse(getCode.data[contractIndex]);
